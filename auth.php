...
function retrieve_password() {
	...
	$message .= network_site_url( "auth.php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . "\r\n";
...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		 ...
		 * as the current user is not set, see vendor/pluggable.php.
	...
	default:
		...
		if ( isset( $_REQUEST['redirect_to'] ) ) {
			...
			if ( $secure_cookie && false !== strpos( $redirect_to, 'panel' ) ) {
		...
		if ( ! is_wp_error( $user ) && ! $reauth ) {
			...
			if ( ( empty( $redirect_to ) || 'panel/' === $redirect_to || admin_url() === $redirect_to ) ) {
