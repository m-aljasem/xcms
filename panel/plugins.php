...
if ( $action ) {
	...
	switch ( $action ) {
		...
		case 'update-selected':
			...
			wp_enqueue_script( 'updates' );
			require_once ABSPATH . 'panel/admin-header.php';
		...
		case 'error_scrape':
			...
			/** This action is documented in panel/includes/plugin.php */
		...
		case 'delete-selected':
			...
			require ABSPATH . 'panel/update.php';
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				...
				/** This action is documented in panel/edit.php */
...
if ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) {
	get_current_screen()->add_help_tab(
		array(
			...
			'content' =>
					'<p>' . __( 'Auto-updates can be enabled or disabled for each individual plugin. Plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the cron.php task scheduling system.' ) . '</p>' .
