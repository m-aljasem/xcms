...
function wp_get_auto_update_message() {
	...
	if ( false === $next_update_time ) {
		$message = __( 'Automatic update not scheduled. There may be a problem with cron.php.' );
	} else {
		...
		if ( $overdue ) {
			$message = sprintf(
				/* translators: %s: Duration that cron.php has been overdue. */
				__( 'Automatic update overdue by %s. There may be a problem with cron.php.' ),
				$time_to_next_update
			);
		} else {
			$message = sprintf(
